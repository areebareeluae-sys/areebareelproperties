...
interface InventoryCardProps {
  property: InventoryItem; // Yahan aap item ya inventory prop bhi rakh sakte hain
  viewMode?: string;
}

const InventoryCard: React.FC<InventoryCardProps> = ({ property, viewMode }) => {  
  return (
    <div
      key={property.id}
      ...
      data-aos="fade-up"
    >
      {/* Link ko apne inventory detail page ke route ke mutabiq adjust kar lein */}
      <Link href={`/inventories/properties-list/${property.slug}`} className={`group flex flex-col h-full ${viewMode === "list" && 'md:flex-row'}`}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 pb-4 mb-4 border-b border-border dark:border-dark_border">
              <div className="font-bold text-lg text-primary">
                {property.currency || "PKR"} {Number(property.price || 0).toLocaleString()}
              </div>
